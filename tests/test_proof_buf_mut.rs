...
fn proof_put_u16() {
    ...
    buf.put_u16(param);
    assert_eq!(param.to_be_bytes(), &buf[..]);

    buf.clear();
    let param2: u16 = kani::any();
    buf.put_u16_le(param2);
    assert_eq!(param2.to_le_bytes(), &buf[..]);
}
...
fn proof_put_int() {
    ...
    buf.put_int(n, sz);
    assert_eq!(&n.to_be_bytes()[(8-sz)..], &buf[..]);
}

#[cfg(kani)]
#[kani::proof]
#[kani::should_panic]
fn proof_put_int_nbytes_overflow() {
    let vsz = kani::any();
    //kani::assume(vsz < 1000000);
    let mut buf = Vec::with_capacity(vsz);
    let sz = kani::any(); 
    kani::assume(sz > 8);
    buf.put_int(kani::any(), sz);
}
